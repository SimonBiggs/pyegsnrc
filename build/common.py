...
#   WARRANTYwithout even the implied warranty of MERCHANTABILITY or FITNESS  
...
APPEND{COMMON/QDEBUG/QDEBUGLOGICAL QDEBUG} TO {COMIN/DEBUG/}
...
#              'average' M and 'average' N shellsphoto-absorption  
...
REPLACE {$CALCULATE_XI(#)} WITH
{
  ...
  IF( spin_effects ) [
      ...
      chia2 = chia2*etapxi = xi*xi_corr
      $EVALUATE ms_corr USING blcce(elkems)
      blccl = blccl*ms_corr
  ]
  ELSE [ xi_corr = 1etap = 1]
...
REPLACE {$CYLNDR(#,#,#,#)} WITH {
   ...
   BCYL=(X(NP)*U(NP)+Y(NP)*V(NP))/ACYLCCYL=X(NP)*X(NP)+Y(NP)*Y(NP)
...
REPLACE {$CONE(#,#,#,#)} WITH {
   ...
   BCON1=(X(NP)*U(NP)+Y(NP)*V(NP))*CPCONBCON=BCON1-ZNP*WNP
...
IF(IPHTER(IR(NP)).EQ.1)[
  ...
  IF(EELEC.GT.ECUT(IR(NP)))[
    ...
    RATIO=BETA/ALPHA
    LOOP[
      $RANDOMSET RNPHTRNPHT=2.*RNPHT-1.
      IF(RATIO.LE.0.2)[
    ...
    IF( iprdst > 0 ) [
        ...
        DO ichrg = 1,2 [
            ...
            IF( iprdst_use = 1 ) [
                ...
                $RANDOMSET COSTHECOSTHE=1.0-2.0*COSTHE
            ...
            ELSE [ sinthe=-sintheNP=NP+1CALL UPHI(3,2)]
        ]
        iq(np) = iq2iq(np-1) = iq1return
    ]
    ELSE[
...
REPLACE {$SELECT_LOW_ENERGY_PAIR_PRODICTION} WITH
{
  ...
  IF( rnno34 < 0.5 ) [ iq1 = -1iq2 = 1] ELSE [ iq1 = 1iq2 = -1]
}
...
# REPLACE {$TURN_OFF_SCATTERING} WITH {cost = 1.000sint = 0.000return}   
...
{
  IF( compute_tstep ) [
    total_de = demfp/sigfedep = total_de
    ekef  = eke - fedep
...
REPLACE {$electron_region_change} WITH {
    ir(np) = irnewirl = irnewmedium = med(irl)
}
...
IRAYLM(J)=1EXIT :LOOP-OVER-REGIONS:]
# END OF REGION-LOOP]
# END OF MEDIA-LOOP]
}
...
# SET FLAG TO PICK UP DATA FOR MEDIUM J AND TRY NEXT MEDIUM.
IPHOTONUCM(J)=1IPHOTONUC=1EXIT :LOOP-OVER-REGIONS-PHOTONUC:]
# END OF REGION-LOOP]
# END OF MEDIA-LOOP]
}
